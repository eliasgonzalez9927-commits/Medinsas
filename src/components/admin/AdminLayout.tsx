import { Building2, ChevronDown, ClipboardList, LogOut, Menu, Search, UserRound, X } from "lucide-react";
...
export function AdminLayout({
  ...
  function navigationSection(groupKey: typeof ADMIN_NAVIGATION_GROUPS[number]["key"], compact = false) {
    ...
    return (
      <section key={groupKey} className={compact ? "" : "mb-5"}>
        <p className="px-3 pb-2 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{label}</p>
            ...
            if (item.status === "coming_soon") {
              return (
                <div
                  ...
                  className="flex cursor-default items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-400"
                  aria-disabled="true"
                >
                  <Icon size={18} />
      ...
      <div className="border-b border-clinic-line px-5 py-5">
        <Link to="/admin" onClick={() => setMobileNavOpen(false)} className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-medium text-clinic-muted">Gestión clínica</p>
          </div>
        </Link>
      </div>

      <div className="border-b border-clinic-line px-4 py-4">
        <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Clínica activa</p>
        <div className="mt-2 flex items-center gap-3">
          <span className="grid h-9 w-9 place-items-center rounded-lg bg-teal-50 text-clinic-brand"><Building2 size={18} /></span>
  ...
  return (
    <div className="min-h-screen bg-[#f6f8fb] text-clinic-ink">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-clinic-line lg:block">{sidebarContent}</aside>
      {mobileNavOpen && (
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-20 border-b border-clinic-line bg-white/95 backdrop-blur">
          <div className="flex min-h-16 items-center gap-3 px-4 sm:px-6 lg:px-8">
            ...
            <Link to="/admin" className="flex items-center gap-2 font-semibold text-clinic-ink lg:hidden"><span className="grid h-9 w-9 place-items-center rounded-lg bg-clinic-brand text-white"><ClipboardList size={19} /></span>Medin</Link>
            ...
            <form className="relative ml-auto hidden w-full max-w-xl md:block lg:ml-0" onSubmit={(event) => { event.preventDefault(); runGlobalSearch(); }}>
              ...
              <input value={globalSearch} onChange={(event) => setGlobalSearch(event.target.value)} placeholder="Buscar código MED, paciente, DNI, teléfono o profesional..." className="h-10 w-full rounded-lg border border-clinic-line bg-clinic-surface pl-10 pr-4 text-sm outline-none transition placeholder:text-slate-400 focus:border-clinic-brand focus:bg-white focus:ring-4 focus:ring-teal-100" />
            </form>
