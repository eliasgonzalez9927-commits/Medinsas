...
import { AutomaticSummaryCard } from "../../../components/admin/AutomaticSummaryCard";
...
export function AttendancePage() {
  ...
  function handleApplyAutomaticSummary(summary: string) {
    const cleanSummary = summary.trim();
    if (!cleanSummary) return;
    setFields((current) => {
      const existing = current.current_condition?.trim() ?? "";
      const appended = existing
        ? `${existing}\n\n— Borrador de resumen automático —\n${cleanSummary}`
        : cleanSummary;
      return { ...current, current_condition: appended };
    });
  }
