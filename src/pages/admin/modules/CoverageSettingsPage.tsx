...
import { HealthCoverage } from "../../../types/clinic";
...
export function CoverageSettingsPage() {
  ...
  async function load() {
    ...
    try {
      const [coverageResult, acceptedResult] = await Promise.all([
        ...
        supabase.from("clinic_accepted_coverages").select("coverage_id, accepted").eq("clinic_id", clinic.id)
      ]);
  ...
  useEffect(() => { if (clinic) load(); }, [clinic?.id]);
  ...
  return <AdminPageShell title="Coberturas aceptadas" eyebrow="Configuración" description="Catálogo operativo de obras sociales, prepagas y agentes del seguro. La reserva no se bloquea si el paciente no tiene cobertura." onRefresh={load}>
    ...
    {clinic && !coverages.length && !error && <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">Todavía no cargaste el catálogo de coberturas. Importá el CSV oficial RNAS desde Importaciones o agregá una cobertura manual.</div>}
    {clinic && <section className="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
