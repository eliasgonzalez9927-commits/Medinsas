...
export function MyPlanPage() {
  ...
  async function load() {
    setError("");
    const current = await getDefaultClinic();
    setClinic(current);
    if (!current) return;
    const [sub, currentUsage, available, requests] = await Promise.all([
      getClinicSubscription(current.id),
      getClinicUsage(current.id),
      supabase.from("subscription_plans").select("*").eq("active", true).order("monthly_price"),
      supabase.from("plan_change_requests").select("id, requested_plan_id, status, created_at").eq("clinic_id", current.id).eq("status", "pending").order("created_at", { ascending: false })
    ]);
  ...
  useEffect(() => { load().catch((err) => setError(err instanceof Error ? err.message : "No pudimos cargar tu plan.")); }, []);
